...
def build_global_glossary(entries, top_anchor, output_path, title, preserve_case=False):
    ...
    top_anchor: a unique string for this file, e.g. "term-index" or "code-index".
                We'll use it to namespace all letter anchors: top_anchor + "-a", etc.
    """
    ...
    with output_path.open("w") as f:
        # --- Top of file: define a plain HTML anchor + heading ---
        # This is what "Back to Top" will link to: #term-index or #code-index
        f.write(f'<a id="{top_anchor}"></a>\n')
        f.write(f"# {title}\n\n")
        f.write("<!-- AUTO-GENERATED: Do not edit this file directly -->\n\n")

        # --- Index with namespaced anchors ---
        ...
        for letter in all_letters:
            ...
            # Define an HTML anchor for the letter section
            ...
            for entry, file in sorted(
                ...
                # MyST label for the term (used by chapter “New in this chapter” links)
                f.write(f"({slug})=\n### {display_term}\n\n")
                if body:
                    f.write(body + "\n\n")

                # Footer: Back to Top uses plain hash link to the top_anchor
                if link_html:
